...
var app = new Vue({
    ...
    methods: {
        ...
        search: function search() {
            ...
            axios.post('/organizations/search/', data).then(function (response) {
        ...
        removePopup: function removePopup(item) {
            ...
            setTimeout(() => $("#remove-organizations").modal("show"), 100);
        },
        remove: function remove() {
            var self = this;
            if (self.operationalLoader === true) return;
            self.operationalLoader = true;
            axios.post('/organizations/remove/' + self.operationalItem.id).then(function (response) {
                var itemIndex = self.items.indexOf(self.operationalItem);
                self.items.splice(itemIndex, 1);
                $("#remove-organizations").modal("hide");
        ...
        changeStatus: function changeStatus(item) {
            ...
            axios.post('/organizations/change-status/' + item.id, bodyFormData).then(function (response) {
